...
parser.add_argument(
    ...
    "--tokenizer",
    default=None,
    help="Path to custom tokenizer.json (uses tiktoken if not provided)",
)
...
# Load tokenizer: custom or tiktoken
if args.tokenizer:
    from tokenizers import Tokenizer

    enc = Tokenizer.from_file(args.tokenizer)
    eot = enc.token_to_id("<|endoftext|>")
    ...
    def tokenize(doc):
        ...
        assert (0 <= tokens_np).all() and (tokens_np < 2**16).all(), (
            f"Token ID out of uint16 range"
        )
    ...
    for tokens in pool.imap(tokenize, dataset, chunksize=16):
        if token_count + len(tokens) < shard_size:
            ...
            filename = os.path.join(
                DATA_CACHE_DIR, f"{hf_dataset_name}_{split}_{shard_index:06d}"
            )
